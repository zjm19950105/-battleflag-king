...
- 运行时属性结算已统一 clamp 到 0 以上；`快速施法` 等 flat/ratio debuff 不会让 `Str/Mag/Def/MDef/Spd/Hit/Eva/Crit/Block` 显示或计算为负数，AP/PP 仍按 0..4 资源上限 clamp。
...
- 治疗最大 HP 路径已统一：`RecoverHp`、`HealRatio`、BattleEnd 治疗和 `HealRatio.lowHpThreshold` 都使用目标装备后的 `GetCurrentStat("HP")`；legacy `Heal25` 也保持使用装备后最大 HP，不要回退 `Data.BaseStats["HP"]`。
- 新增/确认的 active 收尾机制：
...
- 追加战斗语义修复：
  ...
  - 命中率语义已按原版倍率修正：`ActiveSkillData.HitRate` 表示技能命中倍率，公式为 `floor((攻击者命中 - 防守者回避) * 技能命中倍率 / 100)`，地上近接攻击飞行目标时先半减命中差值，最终 clamp 到 0..100；日志同步显示倍率公式。
  - 伤害结果已记录真实扣血前后 HP：`DamageResult.RecordHpChange(...)` 在扣血阶段写入 `DamageReceiverHpBefore` / `DamageReceiverHpAfter` / `AppliedHpDamage`，`DeathResist` 保留 1 HP 时日志不再用 `CurrentHp + damage` 反推。
  - pending action 日志顺序已调整：当前主动行动触发的 `[Counter]` / `[Pursuit]` 会在“行动结束”前结算并显示真实 HP 变化；`StepOneAction` 仍保留兜底 drain。
- 最近提交：
